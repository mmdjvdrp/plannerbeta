...
function startLiveStopwatch() {
  ...
  liveStopwatchInterval = setInterval(() => {
    ...
    if(el && state.liveSession) {
      let diff = parseTime(getNow()) - state.liveSession.sMins; if(diff<0) diff+=24*60;
      if(isPom && diff>=25 && !notified) { notified=true; alert("🍅 ۲۵ دقیقه گذشت! استراحت کنید."); }
      el.innerHTML = `سپری شده: <b>${fmtDur(diff)}</b> ${isPom?'(پومودورو) 🍅':''}`;
    }
  }, 60000);
}
...
export function updateLiveButton(){
  ...
  if(state.liveSession){
    ...
    status.innerHTML = `<div id="live-elapsed-time">در حال محاسبه...</div>`;
    startLiveStopwatch();
  } else {
    btn.classList.remove('is-running'); btn.textContent = 'شروع فعالیت زنده';
    status.textContent = '';
    if(liveStopwatchInterval) clearInterval(liveStopwatchInterval);
  }
}
